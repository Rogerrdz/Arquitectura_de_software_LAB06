...
public final class ControlFrame extends JFrame {
  ...
  private void onPauseAndCheck(ActionEvent e) {
    ...
    manager.pause();
    try {
      // Wait until all threads are actually paused before reading health
      manager.waitUntilAllPaused();
      ...
      for (Immortal im : pop) {
        int h = im.getHealth();
        sum += h;
        sb.append(String.format("%-14s : %5d%s%n", im.name(), h, h <= 0 ? " (DEAD)" : ""));
      }
      sb.append("--------------------------------\n");
      long expected = manager.expectedTotalHealth();
      sb.append(String.format("Total Health (actual)  : %d%n", sum));
      sb.append(String.format("Total Health (expected): %d%n", expected));
      long diff = sum - expected;
      sb.append(String.format("Difference             : %d ", diff));
      if (diff == 0) {
        sb.append("✓ INVARIANT MAINTAINED\n");
      } else {
        sb.append("✗ INVARIANT VIOLATED\n");
      }
      sb.append("Total Fights           : ").append(manager.scoreBoard().totalFights()).append('\n');
      sb.append("Alive Count            : ").append(manager.aliveCount()).append('\n');
      output.setText(sb.toString());
    } catch (InterruptedException ie) {
      Thread.currentThread().interrupt();
      output.setText("Pause interrupted: " + ie.getMessage());
    }
  }
